"""
run_pipeline_local.py — Clara AI Pipeline entry point for LOCAL HF Model.

Usage:
  python scripts/run_pipeline_local.py              # run both Pipeline A and B (default)
  python scripts/run_pipeline_local.py --mode a     # Pipeline A only: demo transcripts -> v1
  python scripts/run_pipeline_local.py --mode b     # Pipeline B only: onboarding transcripts -> v2
"""
...
# Force local mode early so imports pick it up properly
...
def _parse_args():
    parser = argparse.ArgumentParser(description="Clara AI Pipeline (Local Mode)")
    parser.add_argument(
        "--mode",
        choices=["a", "b", "both"],
        default="both",
...
def run_pipeline():
    ...
    print("=" * 50)
    print("  Clara AI Pipeline (LOCAL ENGINE)")
